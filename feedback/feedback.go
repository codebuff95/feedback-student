...
type Feedback struct {
	...
	Textquestions []TextQuestionResponse `bson:"textquestionresponses"`
...
type FeedbackPage struct {
	...
	Textquestions    *[]question.Question
	FormSid          *string
	Collegename      *string
}
...
func parseFeedbackForm(w http.ResponseWriter, r *http.Request, sectionId string) {
	...
	myTextQuestions := &question.GlobalTextQuestions

	var myFeedback Feedback
	myFeedback.Sectionid = sectionId
	myFeedback.Ratings = make([]Rating, len(*myTeachers))
	myFeedback.Textquestions = make([]TextQuestionResponse, len(*myTextQuestions))
	...
	//Recording TextQuestionResponses.
	for i, myTextQuestion := range *myTextQuestions {
		myFeedback.Textquestions[i].Questionid = myTextQuestion.Questionid
		myFeedback.Textquestions[i].Response = template.HTMLEscapeString(r.Form.Get("textresponse~" + myTextQuestion.Questionid))
	}
...
func displayFeedbackForm(w http.ResponseWriter, r *http.Request, sectionId string) {
	...
	if len(question.GlobalTextQuestions) > 0 {
		myFeedbackPage.Textquestions = &question.GlobalTextQuestions
	}
